...
**2026-05-06 — §4.4 mobile unit control sub-pass R1 SCOPE LANDED.** 4-round per-property queue locked: R1 scope (this file) → R2 control-model direction lock + body (one of (i) waypoint+mode-flags / (ii) attack-move-only / (iii) hybrid-with-tier-gated-micro; mode-flags catalog; engagement-defaults shape; control-surface 4-tier scaling principle) → R3 spec-grammar authoring (movement-speed-per-tier shape against §4.11.2 (f) `f_base = 50 u/s` + archetype multipliers; pathing-mode catalog; engagement-rule catalog; falsifiable against §3.4 floor + §4.11.2 (f) + R2 direction) → R4 RN cross-arc audit (3-axis: §4.1 + §4.4a + §3.4 floor / §4.11.2 (f) magnitude consistency / control-surface tier-scaling at T1-T4) + `phase-4.md §4.4` spine-doc edits + `§4.8` exit-gate item #5 ✅ tick. 3x debug loop on axis ran inline (Loop-1 cross-property fragmentation + non-(i) brittleness; Loop-2 (i) is prose-leading candidate per §4.4 explicit endorsement + (ii) collapses engagement-rules grammar §4.4 needs-list mandates + (iii) re-introduces on-field-hero adjacency superseded 2026-04-27; Loop-3 Axis A with embedded cross-property coherence hooks at close of R2/R3). Hard guards locked verbatim: §5.4 + §2.4a [LOCKED] / 2026-04-25 content-skeleton verbatim / 2026-04-26 attack-type lock / §4.1 commander-as-summoned-ability-avatar (no controllable on-field Commander) / §4.4a Builder explicitly out-of-scope / §4.11.2 (f) magnitudes read-only / §3.4 "not full RTS per-unit micro" floor / per-commander R1-R5 lane locks (Hard) / per-tower R1-RN read-only / per-civ R1-RN read-only / three-civ-three-equation-form fingerprint (control surface neutral across all three) / §4.7 Option C + R11 wave-composition variance grammar (LOCKED 2026-05-06) read-only / Cultural-sensitivity Follow-up #5 hard-gate (mechanical-content only). Dual-push points: after R1 / R2 / R3 / RN. PM autonomy mandate carries — R2 onward autonomous. ([`decisions/2026-05-06-mobile-unit-control-r1-scope.md`](decisions/2026-05-06-mobile-unit-control-r1-scope.md))
...
*Document version: 0.77 — 2026-05-06: §4.4 mobile unit control sub-pass R1 SCOPE LANDED. 4-round per-property queue locked (R1 scope → R2 control-model direction lock → R3 spec-grammar authoring → R4 RN cross-arc audit + spine-doc edits + §4.8 exit-gate item #5 tick). 3x debug loop on axis ran inline; (i) waypoint+mode-flags is prose-leading candidate. Hard guards locked verbatim including §4.1 commander-as-summoned-ability-avatar + §4.4a Builder explicitly out-of-scope + §4.11.2 (f) magnitudes read-only + §3.4 "not full RTS" floor + §4.7 Option C + R11 grammar read-only + Follow-up #5 mechanical-content-only gate. Dual-push points: after R1 / R2 / R3 / RN. PM autonomy mandate carries. Prior 0.76 — §4.7 RN arc CLOSED 4/4. Older versions archived to [`CASCADE-history.md`](CASCADE-history.md).*

...
**2026-05-05 — Handoff prep for prototype UI verification sweep (A→B sequence).** Per-commander arc CLOSED 5/5 (`6069c49`); splash-fix landed (`90f6f1c`); first handoff filed (`2edb70c`). PM directive this turn: prepare a second handoff that explicitly sequences NEXT-SESSION work as **(A) static-analysis UI sweep first** — read code, mark every interactive element OK / (Test) / NEEDS-FIX, file `prototype/UI-VERIFICATION-2026-05-MM.md` — **then (B) live in-browser sweep** via Claude-in-Chrome / preview MCP, augment Test→OK or Test→NEEDS-FIX, fix small NEEDS-FIX items inline, queue medium/large to AskUserQuestion. Both A and B happen NEXT session; this turn is handoff-only. **Cadence guardrails per PM ask:** (1) avoid API overload — batch parallel reads, prefer Grep over full-file reads, don't re-read files in context; (2) soft context budget — archive findings to report file not chat context, use Edit not Write where possible, summarize-and-discard intermediate scratch; (3) commit cadence — per ~3-scene batch during A; per ~3 fixed-NEEDS-FIX batch during B; dual-push every commit; (4) fix-in-flight discipline — small fixes inline, medium/large queued. PROGRESS trimmed to 3 entries (Numbers-phase R10 archived). CASCADE pointer single block. Version footer 0.61 → 0.62. HANDOFF.md rewritten with A→B sequence + cadence guardrails + scene checklist preserved + roadmap preserved. No spine-doc edits, no game code edits this turn.

*Original arc-close + first-handoff pointer archived to [`CASCADE-history.md`](CASCADE-history.md).*

**Original arc context (kept for reference):** Arc dual-pushed (`6069c49`); splash-fix dual-pushed (`90f6f1c` — `ATTACK_TYPES` const collision renamed to `CODEX_DATA`, restoring all event-listener wiring). Doc-hygiene: PROGRESS trimmed to 3 entries; CASCADE pointer single block; version footer 0.60→0.61. NEXT-SESSION primary directive flipped to **prototype UI verification sweep** — exhaustive button/command/option review across all 11 scenes (splash/login/menu/profile/mode-select/lobby-coop/commander-pick/tutorial/match/end/options) with `(Test)` markers for unsure or needs-refinement items. Roadmap (per-tower / per-civ / per-map / Phase 5 readiness / research/06-tw-subgenres / admin migration / Follow-ups #5/#6/#7/#8/#9 / C7.b deferred) preserved in HANDOFF. **Original arc context (kept for reference):** Post-CONCEPT-amendment-arc fork: PM picked per-commander authoring (Recommended) over per-tower / per-map / per-civ / Phase-5-readiness / research / admin tracks. **Round 1** filed scope decision + §-placement table + 5-round queue with 3x debug loop on round-structure axis (Axis A per-commander rounds / Axis B per-ability-slot rounds / Axis C per-effect-type rounds). **Axis B chosen** — per-ability-slot, all 3 commanders per round; strongest equivalent-impact-to-§4.11.6-damage-floor discipline (cross-commander parity-check happens every round on the same slot). Axis A rejected (locks commander 1 before commander 2/3 reveal boundary cases). Axis C rejected (compresses commander↔effect-type assignment fork into R1, where it would burn the round on a meta-decision before authoring starts). Loop-1/2/3 synthesis added per-commander identity readings at the close of R2/R3/R4 to mitigate commander-identity fragmentation across rounds. **Round queue locked:** R2 all 3 passive variants (Spartan Training / Blood Tribute / Sons of Ragnar) authored against +15% damage benchmark; R3 all 3 short-CD active variants (This Is Sparta! / Sun Offering / Berserk) against 2,880-burst benchmark + 30s-CD-band discipline; R4 all 3 signature variants (The Last Stand / The Great Sacrifice / The Great Heathen Army) against instant-tier-up benchmark + once-per-match + signature-excluded-from-uptime preserved (per §4.10.5); R5 cross-commander balance audit (9-spec parity check + match-impact simulation against §4.11.8 Hard expert thresholds) + 3 per-commander one-pagers (Leonidas / Montezuma II / Ragnar Lothbrok) + `phase-4.md §4.1` rewrite (mechanical-content body) + `§4.11.6` deferral-language removal + `§4.8` exit-condition gate item #1 tick. Dual-push checkpoints: after R3 (carries R1-R3), after R5 (carries R4-R5 + closes arc). **Locked ability names from `phase-3.md §3.2` carry forward unchanged** — this arc binds *mechanical content* under those names, never re-names. **§4.11.6 damage-floor magnitudes** are the equivalent-impact calibration target, never overwritten — variants authored alongside, not instead. **Cultural-sensitivity Follow-up #5** continues to gate art / pose / VO direction; this arc is mechanical-content only (effect-type / magnitude / duration / CD / target-class — civ-neutral language). §5.4 + §2.4a [LOCKED] + 2026-04-25 locked content skeleton + 17-item frame + all Numbers-phase magnitudes + all R1-R7 amendment-pass §-anchors untouched. Filed [`decisions/2026-05-05-per-commander-effect-type-variant-authoring-scope.md`](decisions/2026-05-05-per-commander-effect-type-variant-authoring-scope.md) (Medium). **R2 LANDED** — passive variants authored across all 3 commanders against +15% damage benchmark. Effect-type assignment locked: Leonidas=Control (15% slow / 2-cell aura / 50% soft-cap), Montezuma II=Economy (+15% bonus Tribute per Aztec-tower kill in aura, multiplicative w/ §4.6a Economy Bonus aux), Ragnar=Summon (one Son @ 40 DPS / 20s cadence / Slashing / HP=2× HP_std(R)). Cross-commander parity table targets ±10% at Hard expert thresholds; per-tower target-side hook interfaces pre-specified for cross-arc parity. **Aura-anchor sub-decision deferred to R5** §4.1 rewrite (cascade drift: §4.1 says "board-wide invisible" / §4.11.6 says "2-cell aura"; 3 resolution candidates flagged: anointed-tower / Commander-pedestal / board-wide-civ-matched). R2 authored against §4.11.6 (more recent + Specification-phase) with `[anchor TBD R5]` placeholders. Filed [`decisions/2026-05-05-per-commander-r2-passive-variants.md`](decisions/2026-05-05-per-commander-r2-passive-variants.md) (Medium). **R3 LANDED** — short-CD active variants authored across all 3 commanders against 2,880-burst benchmark + 30s base CD discipline. Effect-type lanes locked from R2: Leonidas This Is Sparta! = Control (4s hard-stun + 1-cell knockback in 3-cell radius, ~2,700 marginal damage from extended engagement-time on stationary targets, −6% vs floor), Montezuma II Sun Offering = Economy (4s window + 4-cell radius, +35% bonus Tribute on Aztec-tower kills in zone, ~46 T/cast at R20 baseline, multiplicative w/ §4.6a Economy aux for full-stack ~1.6875× per kill in overlap), Ragnar Berserk = Summon (2 Berserkers @ 360 DPS × 4s lifespan, Slashing + frontal cleave + Bleed status-proc, 1× HP_std(R), exact 2,880 burst parity). Cross-commander parity table flags **Sun Offering amortization** for R5 audit (single-cast flatters Economy ~5,500 vs 2,880 nominal floor; full-match impact ~99K vs damage-floor ~52K — intended Aztec scaling identity but R5 must verify Hard 100% breakpoint not breached). Cross-arc per-tower target-side hooks specified: `aztec_tower.bonus_tribute_yield_in_active_zone` flag + global `summons_alive_max = 3` cap + Berserker Slashing-Bleed status-proc cross-ref to 2026-04-26 attack-type-mapping. Per-commander identity readings closed across (h)+(g): Greek lockdown / Aztec scaling / Norse layered-warrior. Filed [`decisions/2026-05-05-per-commander-r3-short-cd-active-variants.md`](decisions/2026-05-05-per-commander-r3-short-cd-active-variants.md) (Medium). **Dual-push checkpoint** after R3 carries R1+R2+R3 per scope decision cadence. **R4 LANDED** — signature variants authored across all 3 commanders against instant-tier-up benchmark (~36K damage-equivalent midpoint, ±15% band per signature-tier scope). Effect-type lanes locked from R2-R3 across (h)+(g)+(i): Leonidas The Last Stand = Control (12s persistent zone-denial in 4-cell radius; hard-stun all enemies present at cast + entering during 12s window; ~28,800 marginal damage, −20% within band; lockdown-strong on Hardcore (k)=1.22 per RPS positioning), Montezuma II The Great Sacrifice = Economy (instant 600 T lump + permanent +10% Aztec-tower yield board-wide for rest of match; full-Aztec-stack post-Sacrifice in active overlap = base × 2.139× per kill — escalates R3 amortization asymmetry to signature scale; pre-bracketed for R5 tune-down if Hard breakpoint or PvP winrate breached), Ragnar The Great Heathen Army = Summon (8 Heathen Warriors @ 180 DPS × 12s lifespan, Slashing + frontal cleave + Bleed status-proc, 1.5× HP_std(R), auto-advance toward path frontline; ~30,000 burst-equivalent + significant DOT propagation; signature-window summon-cap exception 3→9 for 12s only, returning to 3 at despawn). All 3 once-per-match free per §4.10.5 signature-excluded-from-uptime. Per-commander identity arcs CLOSED across all 3 ability slots: **Greek lockdown** (slow + stun + persistent zone in compounding overlap), **Aztec scaling** (passive +15% × active +35% × signature +10% permanent × Economy aux +25% = base × 2.139× compounding), **Norse layered called-warriors** (1 sustained Son + 2 burst Berserkers + 8 mass-arrival Heathen Warriors, all Slashing). Cross-arc per-tower target-side hooks specified across full R2-R4 sweep: `aztec_tower.bonus_tribute_yield_in_active_zone` + `aztec_tower.permanent_tribute_yield_multiplier` + global `summons_alive_max = 3` w/ signature-window exception + Berserker/Heathen Warrior Slashing-Bleed cross-ref to 2026-04-26 attack-type-mapping. R5 audit flags: Aztec full-stack vs Hard 100% breakpoint + PvP-IW winrate; Last Stand duration vs Hardcore; Heathen Army Bleed propagation + summon-cap exception authoring; §4.6a Send-Creeps cross-mode currency attribution. Filed [`decisions/2026-05-05-per-commander-r4-signature-variants.md`](decisions/2026-05-05-per-commander-r4-signature-variants.md) (Medium, 3 identity-coherence guards). **R5 LANDED — ARC CLOSED.** Cross-commander balance audit (9-spec parity check + Hard+Hardcore expert simulation + PvP-IW winrate sanity) surfaced Aztec breakpoint breach (~110-140% of Hard 100%); **Lever-2 applied** — Great Sacrifice lump 600→300 T + permanent yield +10%→+5%, brings Aztec to ~88-104% in band and PvP-IW winrate to 50-58% Aztec favor. **Aura-anchor sub-decision resolved** via 3x debug loop synthesis: anointed-tower with auto-anoint fallback + free re-anoint UX (preserves spatial-play identity for Greek lockdown / clean summon-spawn anchor for Norse / concrete center for Aztec yield bonus). Active-cast targeting decoupled from aura center (player-aimed cell, not anointed-tower). **3 per-commander one-pagers** filed (Leonidas/Montezuma II/Ragnar Lothbrok) closing §4.8 exit-gate item #1. **Spine-doc edits APPLIED** to `concept/phase-4.md`: §4.1 anointed-tower aura model NEW subsection (mechanics + auto-anoint + re-anoint + active-cast vs passive-aura + summon-cap signature-window exception + per-commander effect-type lane-lock table) + Surface table "passive effect" row reworded (anointed-tower-anchored aura, not board-wide invisible) + Non-goals bullet reworded (no commander-avatar persistent — passive aura *does* exist) + §4.1 header tag changed to "per-commander writeup CLOSED 2026-05-05" + §4.11.6 deferral language replaced with R2-R5 outcomes + Lever-2 note + §4.8 exit-gate item #1 ticked DONE. Filed [`decisions/2026-05-05-per-commander-r5-audit-and-arc-close.md`](decisions/2026-05-05-per-commander-r5-audit-and-arc-close.md) (Medium). Decisions table row added. **Dual-push checkpoint** carries R4 + R5 + spine-doc edits per scope decision cadence. cascade-lint expected clean (68 files / 68 rows).

*Prior pointer (2026-05-05 CONCEPT.md amendment pass ARC CLOSED Rounds 1-7) already archived to [`CASCADE-history.md`](CASCADE-history.md).*
...
*Document version: 0.62 — 2026-05-05: second handoff prep — A→B sequence + cadence guardrails. PM directive: NEXT-SESSION work explicitly sequenced as **(A) static-analysis UI sweep first** (read code, mark every interactive element OK / (Test) / NEEDS-FIX, file `prototype/UI-VERIFICATION-2026-05-MM.md`) **then (B) live in-browser sweep** via Claude-in-Chrome / preview MCP (augment Test→OK or Test→NEEDS-FIX, fix small NEEDS-FIX items inline, queue medium/large to AskUserQuestion). Cadence guardrails added: avoid API overload (batch parallel reads, prefer Grep over full-file reads, don't re-read files in context); soft context budget (archive findings to report file not chat context, use Edit not Write where possible); commit per ~3-scene batch during A and per ~3 fixed-NEEDS-FIX batch during B; dual-push every commit. PROGRESS trimmed to 3 entries (Numbers-phase R10 archived); CASCADE pointer single block (per-commander arc-close + first-handoff pointer archived); footer trimmed (0.60 archived). HANDOFF.md rewritten with A→B sequence + cadence guardrails + scene checklist preserved + roadmap preserved verbatim. No spine-doc edits, no game code edits. Prior: 0.61 — 2026-05-05: handoff hygiene + prototype splash-fix. Per-commander effect-type-variant authoring arc closed (`6069c49`) and prototype splash-fix (`90f6f1c` — `ATTACK_TYPES` const collision renamed to `CODEX_DATA`, restoring all event-listener wiring) both dual-pushed. PROGRESS trimmed to 3 entries (Numbers-phase R9 archived to `PROGRESS-archive.md`); CASCADE pointer single block (per-commander arc-close pointer archived to `CASCADE-history.md`); footer trimmed to 2 most recent (0.59 archived). HANDOFF.md rewritten — NEXT-SESSION primary directive flipped from "PM scopes post-arc fork" to **prototype UI verification sweep** per PM ask: exhaustive button/command/option review across all 11 scenes (splash/login/menu/profile/mode-select/lobby-coop/commander-pick/tutorial/match/end/options) with `(Test)` markers for unsure or needs-refinement items. Roadmap (per-tower / per-civ / per-map / Phase 5 readiness gate / research/06-tw-subgenres / admin/concept.json migration / Follow-ups #5/#6/#7/#8/#9 / C7.b deferred) preserved verbatim in HANDOFF state-snapshot section. Prior: 0.60 — 2026-05-05: per-commander effect-type-variant authoring sub-pass ARC CLOSED 5/5 (R1-R5 LANDED). R5 cross-commander balance audit surfaced Aztec breakpoint breach (~110-140% Hard); **Lever-2 applied** (Great Sacrifice 600→300 T + +10%→+5%) → ~88-104% in band + 50-58% PvP-IW Aztec winrate. **Aura-anchor sub-decision resolved** via 3x debug loop: anointed-tower with auto-anoint fallback + free re-anoint UX. **3 per-commander one-pagers** filed closing §4.8 exit-condition item #1. **Spine-doc edits APPLIED** to `concept/phase-4.md` (§4.1 mechanical-content rewrite + §4.11.6 deferral removal + §4.8 exit tick). Older version history (0.59 / 0.58 / 0.57 / 0.56 / 0.55 / 0.54 / 0.53 / 0.52 / 0.51 / 0.50 / 0.47 / etc.) archived to [`CASCADE-history.md`](CASCADE-history.md).*

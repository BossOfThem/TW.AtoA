...
**2026-05-05 — Phase B of prototype UI verification sweep LANDED (`2d2a406`, dual-pushed).** Live in-browser sweep via preview MCP (port 8766; 8765 occupied — added `.claude/launch.json` for future sessions). All 5 small NEEDS-FIX from Phase A landed inline + verified live (commit `1b81f45`): end "Highest Age" undefined row dropped, tutorial Esc → menu wired, `saveAndExit` toast 3-branch refactor (skirmish + co-op explicitly do-not-autosave), `menu-last` resolves civ via `COMMANDERS.roster[cid].civ` w/ legacy fallback, `CAST_DURATIONS.long` dead key removed. **Medium fix #7 landed via AskUserQuestion** (commit `2d2a406`; PM picked Recommended): `renderProfileScene` ln 1086-1099 sources from `Object.entries(COMMANDERS.roster).filter([_,c]=>c.playable)` so civ commanders Leonidas/Montezuma II/Ragnar surface on Profile scene with civ-derived tilt; legacy 5-lineage shape retained as fallback. All fixes verified in-browser before commit; console clean throughout (no SyntaxError, no failed fetches). Report carrier `prototype/UI-VERIFICATION-2026-05-05.md` updated in place — 6 NEEDS-FIX rows flipped to OK; small-fix queue marked "ALL LANDED PHASE B". **Pending queue carried forward:** Medium #6 (input rebind unification — match keydown ln 3537 hardcoded keys; bigger blast radius), Medium #8 (end-screen XP doesn't persist; touches §4.9 save model), Large #9 (`A` age-up shortcut absent — intent call needed), Large #10 (end-screen Continue/Replay/Menu mismatch — markup vs scene-checklist). cascade-lint clean except pre-existing phase-4 soft-cap (626/600). HANDOFF.md rewritten — NEXT-SESSION primary directive flipped to **Phase C — pick from medium/large queue, Recommended-first** (or branch to one of the post-sweep roadmap items). PROGRESS session log appended (3 entries preserved; per-commander arc-close entry archived). CASCADE pointer + footer 0.63 → 0.64. No spine-doc edits this turn.

*Prior Phase A LANDED + handoff-prep pointers archived to [`CASCADE-history.md`](CASCADE-history.md).*
...
*Document version: 0.64 — 2026-05-05: Phase B of prototype UI verification sweep LANDED (`2d2a406`, dual-pushed). Two commits this phase: `1b81f45` (small-fix batch 5/5: end "Highest Age" undefined dropped, tutorial Esc → menu wired, `saveAndExit` toast 3-branch refactor, `menu-last` civ resolution via `COMMANDERS.roster[cid].civ`, `CAST_DURATIONS.long` dead key removed) + `2d2a406` (medium fix #7 — Profile roster sources from `COMMANDERS.roster` filter `playable:true` so civ commanders surface). All fixes verified live in-browser via preview MCP (port 8766). Console clean. `.claude/launch.json` added for preview-MCP server config. Report carrier `prototype/UI-VERIFICATION-2026-05-05.md` updated in place (6 NEEDS-FIX rows → OK). Pending queue carried forward: Medium #6 (input rebind unification) + Medium #8 (end-screen XP persist) + Large #9 (A age-up absent) + Large #10 (end Continue/Replay/Menu mismatch). HANDOFF.md flipped to **Phase C — pick from medium/large queue, Recommended-first**. PROGRESS appended (3 entries preserved; per-commander arc archived to PROGRESS-archive.md); CASCADE pointer single block (Phase A LANDED archived); footer trimmed (0.62 archived). No spine-doc edits. Prior: 0.63 — 2026-05-05: Phase A of prototype UI verification sweep LANDED (`caa1e61`, dual-pushed). Static-analysis sweep across all 11 scenes filed at `prototype/UI-VERIFICATION-2026-05-05.md`: 5 small + 3 medium + 2 large NEEDS-FIX + ~25 (Test) rows. cascade-lint clean except pre-existing phase-4 soft-cap. HANDOFF flipped to Phase B. PROGRESS appended; CASCADE pointer single block; footer trimmed (0.61 archived). Older version history (0.62 / 0.61 / 0.60 / 0.59 / 0.58 / 0.57 / 0.56 / 0.55 / 0.54 / 0.53 / 0.52 / 0.51 / 0.50 / 0.47 / etc.) archived to [`CASCADE-history.md`](CASCADE-history.md).*

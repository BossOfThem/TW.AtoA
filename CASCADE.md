...
**2026-05-04 (later same day) — Balance-pass #1 conceptual frame RATIFIED + Auxiliary economy + game modes ontology PROPOSED.** Follow-up #13 numbers-phase work re-routed: PM picked balance-pass track (Recommended) on next-track gate, then chose to lock the **conceptual frame** before any numbers. Five `AskUserQuestion` rounds (PM Recommended-first picks every round) ratified a 17-item frame covering variable nomenclature (a)–(r), the master damage equation (DPS × engagement-time integral × matchup × passive_modifiers × status_state), the 7-slot round typology (Standard / Swarm / Elite / Modifier / Telegraph / Boss / Recovery) with R5/R10/R15/R20/R25/R30 assignments, single-axis (k) compounding rule (HP-curve exponent only — anti-pattern flag against multi-axis), the 4-step pre-wave-gated sell-refund schedule with T4 cap and (k)-coupling, AI-default + per-tower-override targeting, lump-Tribute + guaranteed-Divinity boss reward shape, the interim Divinity-source pattern (R10/+1 · R20-streak/+1 · R30/+2 · 2 hooks TBD; Foresight-coin Follow-up #7 final), and three skill-bar axes (matchup-rate / placement-coverage / ability-uptime). Decision filed [`decisions/2026-05-04-balance-pass-1-conceptual-frame.md`](decisions/2026-05-04-balance-pass-1-conceptual-frame.md) (Accepted, Medium). Round 5 spawned a Priority-#1 scope expansion: PM rejected flat-interest carry-over and proposed a **modular auxiliary economy structure** (Squadron TD-inspired) plus a **5-mode ontology** (Solo / Horde Co-op / PvE Multiplayer / PvP Interest Wars Lane / PvP Maze Lane). Filed [`decisions/2026-05-04-auxiliary-economy-and-modes-scope.md`](decisions/2026-05-04-auxiliary-economy-and-modes-scope.md) (Proposed, Easy) capturing 10 open ratification axes for Round 6+. CONCEPT-GAPS rows ECON-04 + MODE-01 added. Numbers-phase Follow-up #13 now downstream of frame; consumes the conceptual frame to produce ratified values. cascade-lint clean. §2.4a + §5.4 [LOCKED] untouched. No code edits this turn — concept/math work only.

**2026-05-04 (earlier same day) — C4-continuation polish LANDED + Follow-up #4 closed (Aztec glyph).** First post-graduation session after the C6 slice 2 reshape arc closed. Two tracks ran end-to-end after PM picked C4-continuation polish (Recommended) on the post-graduation AskUserQuestion, then "go" + Aztec-glyph (Recommended) on the follow-up. **C4-continuation polish (commit `c45cb00`, dual-pushed, +64 lines `prototype/index.html`):** action-bar tower-button attack-type stamp pill (sourced from `attackTypeFor(key)`, renders alongside tier-badge + cost); merge-preview ghost extended to fire on hover-over-existing T1/T2 civ tower with same-tier same-civ partner in `CIV_MERGE_RADIUS` (new `hoverHasSameTierCivPartner(t)` helper excludes self; mirrors existing placement-preview ghost logic + reduce-motion branch); T3 civ towers with valid `promoteTargetKey(t)` draw small `↑` indicator badge top-right of hex (gold + pulse when affordable, dim slate otherwise; reduce-motion → static). Right-click context-menu Promote path untouched. **Follow-up #4 (commit `2e56cf5`, dual-pushed):** `prototype/data/civilizations.json` Aztec glyph `𓁹` (Egyptian Eye of Horus, wrong civilization) → `◈` (neutral abstract lozenge). Decision filed [`decisions/2026-05-04-aztec-glyph-abstract-placeholder.md`](decisions/2026-05-04-aztec-glyph-abstract-placeholder.md) (Accepted, Easy). Cultural-sensitivity gate Follow-up #5 explicitly preserved. CASCADE.md decisions table row added. cascade-lint clean throughout. §2.4a + §5.4 [LOCKED] untouched.
...
|------|----------|---------------|
| 2026-05-04 | [Auxiliary economy structure + game modes ontology (Proposed) — Squadron-TD-inspired modular auxiliary structure + 5-mode roster (Solo / Horde / PvE-MP / PvP Interest Wars / PvP Maze) + 10 open ratification axes for Round 6+](decisions/2026-05-04-auxiliary-economy-and-modes-scope.md) | Easy |
...
*Document version: 0.36 — 2026-05-04 (later same day): Balance-pass #1 conceptual frame RATIFIED (17-item frame across 5 PM AskUserQuestion rounds; decision `2026-05-04-balance-pass-1-conceptual-frame.md` Accepted/Medium; numbers-phase Follow-up #13 now downstream of frame) + Auxiliary economy + game modes ontology PROPOSED (decision `2026-05-04-auxiliary-economy-and-modes-scope.md` Proposed/Easy; 10 open ratification axes for Round 6+; CONCEPT-GAPS rows ECON-04 + MODE-01 added). No code edits — concept/math work only. cascade-lint clean. §2.4a + §5.4 [LOCKED] untouched. Prior: 0.35 — 2026-05-04 (earlier): C4-continuation polish LANDED + Follow-up #4 closed (Aztec glyph). [Older version history archived to [`CASCADE-history.md`](CASCADE-history.md).]*

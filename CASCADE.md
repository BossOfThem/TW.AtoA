...
**2026-05-01 — C5 + C7.a + C7.b LANDED (alongside-and-non-breaking pattern, four commits across two slices on C7.b).** Per PM "go" gates per step. Four pure-additive prototype passes, all dual-pushed. **C5 — Codex modal: 7×5 attack-type vs armor RPS matrix** (commit `95165b9`): tutorial + match HUD gain 📖 Codex button → `.codex-modal` with 7-attack-type × 5-armor-tag table; cells colored strong (+25% green) / weak (-25% red) / neutral (1.0×); status-proc legend; bonus/penalty footnote. `ATTACK_TYPES` constant inlined verbatim from `prototype/data/attack-types.json`. ~70 lines JS + 24 lines CSS, pure-additive (no balance wiring, no tower-card mutation). **C7.a — Gut persistent on-field commander avatar** (commit `6063520`): 12 surfaces commented-out (NOT deleted) with dated `// SUPERSEDED 2026-04-27 (C7.a)` markers — initMatch state, 5-field `game.commander` struct, KeyQ + KeyC handlers, net commander-move/-sig handlers, snapshot build/apply, onBoardClick Shift+click branch, towerStats aura mul (replaced with neutral `{damage:1, cd:1, range:1, active:false}`), three commander functions, startWave reset, tick block, render block. Code preserved for C7.b/C6 reference. **C7.b slice 1 — Cast-emerge pipeline** (commit `ae28e1d`): restored `commander: { castState, castTarget, castTtl }` minimal struct + `CAST_DURATIONS = { short: 72, long: 168, signature: 270 }`. `castAbility(slot)` sets state for selected tier (long → signature). Tick decrements + clears at zero. Render: civ-paletted silhouette 1.4× at `game.path[0]` while cast active; signature pulsing ring; reduce-motion → gold ring only. Net snapshot build/apply syncs all three fields. **C7.b slice 2 — BuilderUnit class** (commit `817d048`): `builders: []` array in initMatch; `placeTower` spawn hook pushes `{x, y, target, ttl:120, civ, tier:1}` from path[0] toward placed cell. Tick lerps + decrements + filters. Render: civ-tinted 4px dot + 7px accent ring per builder; legacy commander-a..e use neutral slate fallback. Snapshot build slices to 32 + applies on guest. Builder concurrency cap + 90% refund-on-cancel deferred (no cancel UI). cascade-lint clean throughout. §2.4a + §5.4 [LOCKED] untouched; 2026-04-25 locked content-skeleton names untouched; legacy data + boot loader + commander-a..e all preserved alongside. Next step queued: **C6 — dead-code sweep** (final reshape step; removes superseded primitives + legacy ages.json/towers.json/silhouetteSVG/commander-a..e per alongside-discipline graduation). PM go required.

*Prior pointer block archived to [`CASCADE-history.md`](CASCADE-history.md).*

**Next planned work:**
1. **C6 — dead-code sweep** [next on PM "go"; removes 12 commented-out C7.a markers + legacy 5-lineage data + boot loader + commander-a..e + `silhouetteSVG()` + dead branches].
2. **C4-continuation** — merge-preview ghosts / Promote-to-T4 button / tower-card tier+type stamp (requires civ-tower placement wiring; gated on or interleaved with C6).
3. Cultural-sensitivity pass scheduling (Follow-up #5) — independent track.
...
*Document version: 0.32 — 2026-05-01: C5 + C7.a + C7.b LANDED via alongside-and-non-breaking pattern (four commits 95165b9 / 6063520 / ae28e1d / 817d048, all dual-pushed). C5 = Codex modal (7×5 RPS matrix from attack-types.json, pure-additive); C7.a = gut persistent on-field commander avatar (12 surfaces commented-out with dated markers, neutral aura mul preserves tower math); C7.b slice 1 = cast-emerge pipeline (commander.castState struct + CAST_DURATIONS + civ-paletted silhouette 1.4× at home anchor + reduce-motion fallback + net sync); C7.b slice 2 = BuilderUnit class (builders[] + placeTower spawn hook + lerp tick + civ-tinted render + snapshot sync; concurrency cap + refund-on-cancel deferred). cascade-lint clean throughout. Pointer rewritten; prior 2026-04-30 (continuation) C2/C3/C4-slice block archived. Next step C6 — dead-code sweep. §2.4a + §5.4 [LOCKED] untouched; locked content-skeleton names untouched. Prior: 0.31 — 2026-04-30 (continuation): C2 + C3 + C4 (slice) LANDED via alongside-and-non-breaking pattern (silhouette migration + Tribute/Divinity bar + Fusion menu + Cast bar). [Older version history archived to [`CASCADE-history.md`](CASCADE-history.md).]*

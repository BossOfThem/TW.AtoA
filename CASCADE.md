...
**2026-05-05 — CONCEPT.md amendment pass ARC CLOSED: Rounds 1-7 of 7 LANDED.** Numbers-phase Follow-up #13 closed; PM picked the CONCEPT.md amendment pass as the next post-arc track. **Round 1** filed scope decision + §-placement table + 7-round queue with 3x debug loop on §-placement (Option A single mega-round / Option B per-decision-file / Option C topical 7-round queue chosen). **§-placement locked:** mode-roster expansion → `phase-3.md §3.6` (Round 2); 17-item conceptual frame + extension (s) + master equation → `phase-4.md §4.10` NEW (Round 3); HP/yield/speed/DPS/commander magnitudes/per-mode tuning → `phase-4.md §4.11` Numbers floor NEW (Round 4); Tribute/Divinity economy + auxiliary slot catalog + Divinity sources → `phase-4.md §4.6` rewrite + `§4.6a` NEW (Round 5); skill-bar thresholds + Round 11 mandate + "Go big" doctrine → `phase-4.md §4.7` + `phase-6.md` + `phase-7.md §7.4` (Round 6); hub version bump + phase index touches → `CONCEPT.md` (Round 7). Dual-push checkpoints after Rounds 3 / 5 / 7. §5.4 + §2.4a [LOCKED] + 2026-04-25 locked content skeleton untouched throughout. Filed [`decisions/2026-05-05-concept-amendment-pass-scope.md`](decisions/2026-05-05-concept-amendment-pass-scope.md) (Easy). PM autonomy mandate from Numbers-phase mid-arc carries forward — Rounds 2-7 produce autonomously. **Round 2** rewrote `phase-3.md §3.6` mode roster 5→6 (Solo Campaign / Horde-A / Horde-B / PvE-MP / PvP-IW / PvP-Maze) with per-mode summary table (topology / count / win / (k) / aux surface) + universal-aux recap + per-mode lives + post-launch list + solo-promise audit. §3.4 stale Commander-Hero-Unit residue redacted (2026-04-27 supersession). §3.8 exit-condition mode-count 5→6. Filed [`decisions/2026-05-05-concept-amendment-pass-round-2-phase-3-modes.md`](decisions/2026-05-05-concept-amendment-pass-round-2-phase-3-modes.md) (Easy). **Round 3** added `phase-4.md §4.10 Tower-vs-Unit math NEW` (9 sub-sections: variable nomenclature (a)-(r) + extension (s) / master damage equation / round-slot typology / (k) single-axis rule / skill-bar axes / sell-refund schedule / target-priority / boss-Elite reward shape / engine-port discipline) + `§4.11 Numbers floor` placeholder seeded for Round 4. Inserted between §4.9 and §4.8 per §4.4a precedent. Filed [`decisions/2026-05-05-concept-amendment-pass-round-3-phase-4-tower-vs-unit-math.md`](decisions/2026-05-05-concept-amendment-pass-round-3-phase-4-tower-vs-unit-math.md) (Medium). **Round 4** populated `phase-4.md §4.11 Numbers floor` body in-place (placeholder → 8 sub-sections binding all 2026-05-05 Numbers-phase R1-2-3-4-5-6-8-9 magnitudes): §4.11.1 HP curve `100·1.16^R` + boss multipliers ×3/×2/×5; §4.11.2 (k) exponents 1.16/1.19/1.22; §4.11.3 Tribute `5·1.10^R` + boss lumps 250/400/1500 + 1Div each; §4.11.4 speed `f_base=50` + archetype mults + ε=0.6/N=24; §4.11.5 DPS ladder T1=20→God=1080 + tier-up costs 100/300/900/2500; §4.11.6 commander +15% passive / 2,880 burst @ 30s CD / once-per-match free tier-up; §4.11.7 per-mode tuning + N-scaling + R31+ HP×1.5^(R-30) tie-break; §4.11.8 skill-bar thresholds 60/75/90 · 50/65/80 · 40/60/80 with Hardcore-expert ~0.58× realized. Filed [`decisions/2026-05-05-concept-amendment-pass-round-4-phase-4-numbers-floor.md`](decisions/2026-05-05-concept-amendment-pass-round-4-phase-4-numbers-floor.md) (Medium). **Round 5** rewrote `phase-4.md §4.6 Economy` in-place (drops superseded "every build touches lineage" prose + Mythium placeholder + 5-bullet stale "Open" trailer; adds Tribute curve binding to §4.11.3 + Divinity 6-source table 4-floor + 2-escalation per R10 amendment + explicit Mythium retirement note pointing to kill-only-currency lock + PvP-IW Send-for-Interest as preserved Squadron-TD shape). Authored `§4.6a Auxiliary economy NEW` — universal-aux-slot catalog (7 rows: Damage Bonus / Economy Bonus / Tower-count expansion / Send-Creeps / Send-for-Interest / Call-for-Help / Maze Stone with cost/effect/cap/mode-availability) + currency-budget audit (max-aux+1-Fusion = 5 Div ✓ / max-Fusion+menu = 5 Div ✓ / 1.20 (s) × 1.15 (h) × 1.25 matchup = 1.725× per-tower expert stack) + mode-availability cross-pointer to §4.11.7. §4.10.1 (s) + §4.10.7 forward-anchors from Round 3 resolve live. Phase-4 trimmed to exactly 600 lines. Filed [`decisions/2026-05-05-concept-amendment-pass-round-5-phase-4-economy.md`](decisions/2026-05-05-concept-amendment-pass-round-5-phase-4-economy.md) (Medium). **Round 6** added 3 changes across 3 phase docs: `phase-4.md §4.7` "Wave-composition variance mandate" subsection (random-seeded armor-tag mix + per-map crystal-lock variance per R9 close-out — defends §4.11.8 thresholds from memorization meta; locked content-skeleton bosses exempt); `phase-6.md §6.5 Skill-bar telemetry NEW` (protocol for matchup-correctness from RPS-counter kill events / placement-coverage from tower-engaged-second ratio / ability-uptime from cast-cooldown ratio with signature excluded — resolves §4.11.8 forward-anchor); `phase-7.md §7.4` appended "Go big at launch (non-negotiable)" bullet (MVP ships full content skeleton; cuts go to post-launch never to launch skeleton). §4.6a audit bullets + mode-availability paragraph compressed to free lines for §4.7 R11 addition (phase-4 597 lines). phase-6/phase-7 Last-reviewed 2026-04-26→2026-05-05. Filed [`decisions/2026-05-05-concept-amendment-pass-round-6-phase-4-7-r11-and-phase-6-7.md`](decisions/2026-05-05-concept-amendment-pass-round-6-phase-4-7-r11-and-phase-6-7.md) (Easy). **Round 7** CLOSES arc 7/7. Hub `CONCEPT.md` updated: phase-index rows 3 / 4 / 6 / 7 touched (Phase 3 row += 6-mode roster; Phase 4 row += §4.6a aux-slot catalog + §4.10 Tower-vs-Unit math + §4.11 Numbers floor + Tribute kill-only / Divinity 6-source qualifications; Phase 6 row += §6.5 skill-bar telemetry; Phase 7 row += "Go big at launch" non-negotiable pointer); Non-negotiables list += "Go big at launch" bullet (promoted from §7.4); version footer 0.8→0.9 with full arc summary. §5.4 + §2.4a [LOCKED] UNTOUCHED throughout. Hub vision line UNTOUCHED (already real-cultures-aligned). Filed [`decisions/2026-05-05-concept-amendment-pass-round-7-hub-close.md`](decisions/2026-05-05-concept-amendment-pass-round-7-hub-close.md) (Easy). **All 22 ratifications from 2026-05-04 Balance-pass #1 (12 rounds) + 2026-05-05 Numbers-phase #13 (10 rounds) now resolve to stable §-anchors in the spine docs.** Spine-anchor stability achieved. Post-arc unblocked tracks: per-tower authoring sub-pass / per-commander authoring sub-pass / per-civ specialization / per-map authoring / research/06-tw-subgenres / Phase 5 readiness gate. cascade-lint clean throughout.

*Prior pointer (2026-05-05 Numbers-phase Follow-up #13 ARC CLOSED Rounds 1–10) archived to [`CASCADE-history.md`](CASCADE-history.md).*

**Next planned work (post-amendment-arc; PM scope-pick required):**
1. **Per-tower authoring sub-pass** — bind cd / range / attack-type / status-proc across 18 T1-T3 + 18 T4 Demigod + 9 God towers across 3 civs. Consumes §4.10 frame + §4.11 magnitudes + §4.6a aux catalog. Likely 5-10 rounds.
2. **Per-commander authoring sub-pass** — bind control / summon / economy effect-type variants of 3 (h) slots equivalent-impact to damage-floor. Consumes §4.11.6 + §4.1 identity-floor.
3. **Per-civ specialization** — Greek / Aztec / Norse identity profiles (matchup affinities, identity hooks, signature creep types). Intersects Follow-up #5 cultural-sensitivity gate.
4. **Per-map authoring (incl. Round 11 mandate)** — good-cell authoring + wave-randomization seeds + crystal-lock variance. Defends §4.11.8 thresholds from memorization meta. Cross-cuts all 6 modes.
5. **Phase 5 readiness gate** — engine-side telemetry implementation per §6.5 protocol; wave-composition variance per §4.7 R11.
6. **`research/06-tw-subgenres.md`** new stub — Squadron TD / Legion TD 2 / BTD6 / Element TD / Line Tower Wars / Mini-TD deep dives.
7. **`admin/concept.json` migration direction** — long-deferred. PM picks: full rewrite to 3-civ/4-tier/Fusion shape, regenerate from CONCEPT.md, or retire entirely.
8. **Follow-up #5 cultural-sensitivity pass scheduling.** Hard pose-lock + content-lock gate before any culturally-coded civ art ships.
9. **Follow-up #6** — Patch-1 commanders per civ + Thor Slashing+Crushing recipe-layer dissonance.
10. **C7.b deferred items** — Builder concurrency cap + 90% refund-on-cancel UI.
11. Other follow-ups #7 / #8 / #9 (Foresight-coin / PvE campaign + AGES + leveling + attributes / non-boss enemy ontology).
...
*Document version: 0.55 — 2026-05-05: handoff hygiene — PROGRESS session log appended with CONCEPT amendment pass ARC CLOSED entry + R8 archived; current pointer block trimmed to 1 (Numbers-phase Follow-up #13 prior block archived to CASCADE-history.md); next-planned-work refreshed for post-amendment-arc fork (per-tower / per-commander / per-civ / per-map authoring tracks + Phase 5 readiness + research/admin debts); HANDOFF.md fully rewritten for amendment-arc-closed state. cascade-lint clean. No spine-doc edits. Prior: 0.54 — 2026-05-05: CONCEPT amendment pass ARC CLOSED (Rounds 1-7 of 7 LANDED). Round 7 closes the arc: hub `CONCEPT.md` phase-index rows 3/4/6/7 touched (Phase 3 += 6-mode roster; Phase 4 += §4.6a aux-slot catalog / §4.10 Tower-vs-Unit math / §4.11 Numbers floor / Tribute kill-only + Divinity 6-source qualifications; Phase 6 += §6.5 skill-bar telemetry; Phase 7 += "Go big at launch" pointer); Non-negotiables list += "Go big at launch" bullet promoted from §7.4; version footer 0.8→0.9 with full 7-round arc summary. §5.4 + §2.4a [LOCKED] UNTOUCHED throughout entire arc. All 22 ratifications from 2026-05-04 Balance-pass #1 (12 rounds) + 2026-05-05 Numbers-phase #13 (10 rounds) now resolve to stable §-anchors in spine docs. cascade-lint clean. [Older version history (0.53 / 0.52 / 0.51 / 0.50 / 0.47 / etc.) archived to [`CASCADE-history.md`](CASCADE-history.md).]*

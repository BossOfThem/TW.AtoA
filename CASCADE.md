...
**2026-05-06 — Per-civ specialization sub-pass OPENED. R1 scope decision LANDED** ([`decisions/2026-05-06-per-civ-specialization-scope.md`](decisions/2026-05-06-per-civ-specialization-scope.md)). Track picked via AskUserQuestion (PM picked Recommended both: Axis A per-civ deep-dive + 5-field schema lock). 4-round queue: R2 Greek profile / R3 Aztec / R4 Norse / RN cross-civ × cross-field audit + `phase-4.md §4.x` per-civ specialization body + possible §4.8 exit-gate item add. 3x debug loop on axis (Axis B per-axis cross-civ and Axis C single condensed round rejected; Axis A chosen with embedded cross-civ parity hook paragraphs at close of R2/R3/R4). 5-field schema: matchup_affinity (vs §4.10 5-armor RPS classes) / identity_hook (one-paragraph mechanical thesis synthesizing lane-lock + per-tower magnitude signature + status-proc archetype + commander synergy) / signature_creep_types (armor-class grain pre-§4.7-lock) / multi_civ_play_hook (per-tower RN type-coverage gaps: Greek-no-Poison / Aztec-no-Crushing / Norse-no-Poison-no-Divine-T1-T3) / commander_synergy (per-commander R5 lane-lock binding). Hard guards locked: 2026-04-25 content-skeleton names verbatim / 2026-04-26 attack-type lock / 2026-05-05 R5 baseline DPS ladder (read-only ref) / per-commander R1-R5 lane locks (Hard) / per-tower R1-RN magnitude matrix (read-only ref) / §4.7 OPEN — signature_creep_types stays at armor-class archetype-grain. Cultural-sensitivity Follow-up #5 hard-gates art / VFX / civ-flavor surface direction; this arc is mechanical-content only. Inventory: 3 civs × 5 fields = 15 cells. Dual-push points: after R2 (carries R1+R2) / after R3 / after RN (closes arc). PM autonomy mandate carries forward — surface AskUserQuestion only on genuine forks / scope expansion / cascade-violation risk / cultural-sensitivity / handoff trigger.

*Prior 2026-05-06 Per-tower arc CLOSED pointer + earlier pointers archived to [`CASCADE-history.md`](CASCADE-history.md).*

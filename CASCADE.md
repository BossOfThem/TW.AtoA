...
**2026-04-30 (continuation) — C2 + C3 + C4 (slice) LANDED (alongside-and-non-breaking pattern, three commits).** Per PM "go" gates with bypass-permissions extension through C3 and explicit handoff heads-up before C4. Three pure-additive prototype passes, all dual-pushed. **C2 — Silhouette 5→3 SVG migration** (commit `cd376f1`): added `civSilhouetteSVG()` + `commanderSilhouetteSVG()` + `commanderSilhouetteColor()` alongside legacy `silhouetteSVG()`; civ commanders → civ silhouettes (Greek triangle / Aztec hexagon / Norse diamond), legacy commander-a..e → legacy lineage silhouettes. `CIV_PALETTE` inlined. Cultural-sensitivity gate honored via abstract geometric primitives (no glyphs/poses); skipped civilizations.json glyphs (Aztec entry is an Egyptian eye — flagged Follow-up #4). 3 call sites updated: roster chip / portrait card / HUD silhouette. **C3 — Tribute + Divinity resource bar** (commit `45ff858`): added Tribute (mirror of `game.gold`) + Divinity (new state field, 6-pip cap) + Fusion HUD button (disabled until divinity ≥ 2) in both tutorial and match scenes; `game.divinity` initialized in `initMatch` + `initTutorial` + Net snapshot build/apply; wave-end hook awards +1 Divinity on `wave % 5 === 0` boss rounds. Legacy gold/knowledge HUD + tower buy/upgrade/sell + age-up logic untouched. **C4 (slice) — Fusion menu modal + Commander Cast bar** (commit `ca2a5cf`): self-trimmed under context pressure to pure-additive surfaces. Fusion modal (3 civ columns × 3 god cards reading inline `FUSION_RECIPES` with verbatim PM-locked sources/types; click → `fusionExecute()` decrements divinity by 1 + stub toast). Cast bar (3 buttons populated from `c.stats.passive/shortCdActive/longCdSignature` for civ commanders only; `castAbility(slot)` plays random VO ability bark + decorative 8s/18s CD lockout). **Deferred under alongside discipline:** merge-preview ghosts, Promote-to-T4 button, tower-card tier/type stamp — all require civ-tower placement wiring the legacy 5-lineage tower system lacks; queued as **C4-continuation** post-C5. cascade-lint clean throughout. §2.4a + §5.4 [LOCKED] untouched. Locked content-skeleton names from 2026-04-25 untouched. Legacy data + boot loader + commander-a..e all untouched. Next step queued: **C5 — 7×5 RPS indicator** (or C4-continuation if PM redirects). PM go required.

*Prior pointer block archived to [`CASCADE-history.md`](CASCADE-history.md).*

**Next planned work:**
1. **C5 — 7×5 RPS indicator** [next on PM "go"; render attack-type × armor matrix in tooltip/codex; reads `prototype/data/attack-types.json`].
2. **C4-continuation** — merge-preview ghosts / Promote-to-T4 button / tower-card tier+type stamp (requires civ-tower placement wiring; gated on or interleaved with C5).
3. C7.a → C7.b → C6 per amended order, one PM gate per step.
4. Cultural-sensitivity pass scheduling (Follow-up #5) — independent track.
5. Patch-1 commanders per civ (Follow-up #6).
6. `admin/concept.json` migration path.
...
*Document version: 0.31 — 2026-04-30 (continuation): C2 + C3 + C4 (slice) LANDED via alongside-and-non-breaking pattern (three commits cd376f1 / 45ff858 / ca2a5cf, all dual-pushed). C2 = civ silhouettes (geometric placeholders, cultural-sensitivity gate honored); C3 = Tribute mirror + Divinity 6-pip + Fusion HUD button + wave-end +1 hook; C4 slice = Fusion menu modal + Commander Cast bar (merge-preview/Promote-T4/tower-card-stamp deferred to C4-continuation under context pressure). Legacy data + boot loader + commander-a..e all untouched. cascade-lint clean throughout. Pointer rewritten; prior 2026-04-29 (continuation) C1 pointer block archived. Next step C5 — 7×5 RPS indicator (or C4-continuation if PM redirects). §2.4a + §5.4 [LOCKED] untouched; locked content-skeleton names untouched. Prior: 0.30 — 2026-04-29 (continuation): C1 Data layer reshape LANDED via alongside-and-non-breaking pattern (4 new prototype data files; commanders.json migrated; lint extended). [Older version history archived to [`CASCADE-history.md`](CASCADE-history.md).]*

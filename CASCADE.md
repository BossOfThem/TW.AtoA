...
**2026-05-06 — §4.4 mobile unit control sub-pass RN LANDED (CLOSES ARC 4/4; §4.8 exit-gate item #5 ✅ DONE).** 3-axis cross-arc audit at zero cascade-violations across 12 cells: (Axis 1) §4.1 + §4.4a + §3.4 floor coherence (Commander excluded / Builder excluded / negative-space verbatim / per-rule × per-tier falsification gate all PASS); (Axis 2) §4.11.4 magnitude consistency (read-only verbatim / archetype-multiplier table coverage / T2-T3-share-Standard intentional / civ-neutral all PASS); (Axis 3) control-surface tier-scaling at T1-T4 (mode-flag count tier-invariant / engagement-default count tier-invariant / aggro tie-breaker discipline tier-invariant / negative-space verbatim across tiers all PASS). **Spine-doc edits applied** to `concept/phase-4.md §4.4`: OPEN BLOCKER tag cleared; OPEN ISSUE paragraph replaced with four-paragraph locked body (a) speed-ladder inline table + (b) pathing-mode catalog inline table + (c) engagement-rule catalog inline table with §3.4 floor verification column + (d) no-tier-scaling principle paragraph + civ-neutrality assertion close. **§4.8 exit-gate item #5 ✅ ticked DONE.** Phase 4 has **5 of 11 exit-gate items closed** (#1 commander one-pagers / #2 per-tower spec / #3 per-civ specialization / #4 enemy direction / #5 mobile unit control). 6 items remaining (Fusion-numerics / Economy paper-balance / Monetization specifics / Engine lock / Art director / Cultural-sensitivity Phase-4 exit gate). Hard guards verified untouched verbatim. PM autonomy mandate carries — next-track candidates surface at next PM gate post-RN dual-push. ([`decisions/2026-05-06-mobile-unit-control-rn-cross-arc-audit-and-arc-close.md`](decisions/2026-05-06-mobile-unit-control-rn-cross-arc-audit-and-arc-close.md))
...
*Document version: 0.80 — 2026-05-06: §4.4 mobile unit control sub-pass RN LANDED (CLOSES ARC 4/4). 12-cell 3-axis cross-arc audit at zero cascade-violations: Axis 1 §4.1+§4.4a+§3.4 floor (Commander/Builder excluded; per-rule × per-tier falsification PASS); Axis 2 §4.11.4 magnitude consistency (read-only verbatim); Axis 3 control-surface tier-scaling T1-T4 (zero new buttons per tier). Spine-doc edits applied to `concept/phase-4.md §4.4` (OPEN BLOCKER cleared; four-paragraph locked body with three inline tables) + `§4.8` exit-gate item #5 ✅ ticked DONE. Phase 4 advances to **5 of 11 exit-gate items closed**. Hard guards verified untouched verbatim. PM autonomy mandate carries — next-track candidates surface at next gate post-RN dual-push. Prior 0.79 — §4.4 R3 spec-grammar authoring landed. Older versions archived to [`CASCADE-history.md`](CASCADE-history.md).*

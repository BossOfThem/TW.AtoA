...
**2026-05-06 — Per-tower authoring sub-pass CLOSED (R1 + R2 + R3 + R4 + RN LANDED).** All 45 launch towers (15 Greek + 15 Aztec + 15 Norse, 6 T1-T3 mainline + 6 T4 Demigod + 3 God per civ) bound to the locked 7-field schema (attack_type / cd / range / status_proc / dps / aux_slot_compat / notes). Cross-civ audit at [`RN`](decisions/2026-05-06-per-tower-rn-cross-civ-audit-and-arc-close.md) confirms zero cascade-violations across 4 locks (attack-type / dps-band / proc-kind / lane-tag) × 45 towers. Three falsifiable per-civ magnitude signatures empirically confirmed: **Greek Control** = dps-centered + cd-1.7s-mid + range-4.0-mid + lockdown-procs; **Aztec Economy** = dps-band-high T2-T3 (4 of 6 mainline at +10%) + cd-1.4s-faster + range-4.0-mid + Poison/Fire/Slashing-bleed compounding (3 sources of stacking-Bleed across tiers); **Norse Summon** = dps-centered + cd-1.15s-fastest + range-3.5-shortest + Bleed-density 6/15 (densest single-proc concentration). Spine-doc edits applied: §4.8 exit-gate item #2 ticked DONE (per-tower spec table populated); §4.11.5 closing cross-reference paragraph added. Phase 4 exit-gate items #1 + #2 closed; items #3-#9 carry forward. **Next-track fork** open via AskUserQuestion next session — candidates: per-civ specialization / per-map authoring / §4.4 mobile-unit-control / §4.7 enemy direction / monetization specifics / engine choice lock / art director scope.

*Prior 2026-05-06 Per-tower R1 scope LANDED + 2026-05-05 Phase C LANDED + sweep CLOSED + Phase B + Phase A + handoff-prep pointers archived to [`CASCADE-history.md`](CASCADE-history.md).*

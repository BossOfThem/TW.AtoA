...
**2026-05-06 — §4.4 mobile unit control sub-pass R3 LANDED (spec-grammar authoring).** Four bodies authored at spec-level grain: (A) **Movement-speed-per-tier ladder** binds 4-tier mobile-unit ladder to existing §4.11.4 archetype-multiplier table — T1 swarm = Swarm × 1.5 = 75 u/s / T2 mainline = Standard × 1.0 = 50 u/s / T3 elite = Standard × 1.0 = 50 u/s (T2/T3 share Standard intentionally — tier expression lives in stats + passives per R2 Body-3, not speed) / T4 Demigod = Boss × 0.7 = 35 u/s; no new magnitudes authored (Numbers-phase #13 R4 read-only); Builder exempt per §4.4a; (B) **Pathing-mode catalog enumeration** = R2 Body-1's 4 flags (`Patrol` / `Follow-Lane` *(default)* / `Hold` / `Attack-Move`) bound to one-line spec-level semantics each; group-select rule `group_select(units).waypoint(wp) ≡ for u in units: u.waypoint(wp)`; flags untouched on box-select; UI-affordance grain deferred to Phase 5; (C) **Engagement-rule catalog enumeration** = R2 Body-2's 3 defaults (`R_engage` / `HP_fall_back` / `Hold_fire`) + aggro-priority shape with one-line semantics + per-tier §3.4 floor falsification gate at T1/T2/T3/T4 — all rules pass at all tiers (Loop-2-adopted edit: HP_fall_back anchor selection excludes Builders; walk-and-despawn lifecycle would orphan units); (D) **Aggro tie-breaker resolution (R2-handed-down)** = keep priority shape + stable-sort by spawn-order entity-id + no-override reaffirmation (Loop-2-adopted edit: cost-acknowledged framing — eagle-vs-near-dead-berserker scenario remains structural cost of no-buttons rule, not bug). 3x debug loop validated Body A T2/T3 share is feature; Body B group-select UX deferral; Body C Builder-anchor edit; Body D framing softening. Cross-property coherence hook to R4 RN with 3-axis audit deliverables (Axis 1 §4.1+§4.4a+§3.4 floor / Axis 2 §4.11.4 magnitude consistency / Axis 3 control-surface tier-scaling at T1-T4). Hard guards verified untouched verbatim (§5.4 + §2.4a [LOCKED] / 2026-04-25 content-skeleton / 2026-04-26 attack-type lock / §4.1 commander-as-summoned-ability-avatar / §4.4a Builder excluded / §4.11.4 (f) magnitudes read-only / §3.4 floor / per-commander R1-R5 / per-tower R1-RN / per-civ R1-RN / three-civ-three-equation-form fingerprint / §4.7 Option C + R11 grammar / Cultural-sensitivity Follow-up #5). R4 (RN) next: 3-axis cross-arc audit + `phase-4.md §4.4` spine-doc edits (control-model body merged from R2 Bodies 1-3 + R3 Bodies A-D with inline tables) + §4.8 exit-gate item #5 ✅ tick + arc close. PM autonomy mandate carries. ([`decisions/2026-05-06-mobile-unit-control-r3-spec-grammars.md`](decisions/2026-05-06-mobile-unit-control-r3-spec-grammars.md))
...
*Document version: 0.79 — 2026-05-06: §4.4 mobile unit control sub-pass R3 LANDED (spec-grammar authoring). Four bodies at spec-level grain: (A) speed-ladder binding 4-tier ladder to §4.11.4 archetype multipliers (75/50/50/35 u/s; T2/T3 share Standard intentionally per R2 Body-3); (B) pathing-mode catalog with one-line semantics + group-select rule; (C) engagement-rule catalog with per-tier §3.4 falsification gate (Builder excluded as HP_fall_back anchor — Loop-2 edit); (D) aggro tie-breaker resolution = stable-sort by entity-id + no-override reaffirmation (cost-acknowledged framing — Loop-2 edit). 3x debug loop validated all four bodies. Cross-property coherence hook to R4 RN with 3-axis audit deliverables (§4.1+§4.4a+§3.4 floor / §4.11.4 magnitude consistency / control-surface tier-scaling). Hard guards verified untouched verbatim. R4 (RN) next: 3-axis cross-arc audit + spine-doc edits at `phase-4.md §4.4` + §4.8 exit-gate item #5 ✅ tick + arc close. Prior 0.78 — §4.4 R2 control-model direction lock landed. Older versions archived to [`CASCADE-history.md`](CASCADE-history.md).*

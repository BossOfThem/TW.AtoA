...
**2026-04-29 (continuation) — C1 Data layer reshape LANDED (alongside-and-non-breaking pattern).** Per PM "go" on Recommended option after a single AskUserQuestion clarifying the 11→4 cardinality break on the ages→tiers rename. PM ratified **alongside pattern**: new canonical files land alongside legacy data; legacy stays running until C3-C5 dismantle the age-loop UI; full removal at C6 dead-code pass. Same pattern applied to `commanders.json` (3 civ commanders added alongside legacy 5; all 8 playable for lint). **Files added (4 new):** [`prototype/data/civilizations.json`](prototype/data/civilizations.json) (3 civs Greek/Aztec/Norse with locked 6/5/6/3 content skeleton from 2026-04-25 ratification), [`prototype/data/fusion-recipes.json`](prototype/data/fusion-recipes.json) (9 locked recipes; Thor Slashing+Crushing dissonance reaffirmed as Follow-up #6), [`prototype/data/attack-types.json`](prototype/data/attack-types.json) (7 types + 5 armor tags + RPS matrix + per-tower/demigod assignments per 2026-04-26 attack-type-mapping decision; Poison locked stacking exception), [`prototype/data/tiers.json`](prototype/data/tiers.json) (4-tier ladder + fusion meta-entry isATier:false). **File modified:** [`prototype/data/commanders.json`](prototype/data/commanders.json) — Leonidas / Montezuma II / Ragnar Lothbrok added with locked signature-ability names + epithets + lore + 12-line VO banks + 20-level XP ladders + 3 cosmetic slots; legacy commander-a..e preserved unchanged. **Lint extended:** [`tools/cascade-lint.py`](tools/cascade-lint.py) — schema checks for civilizations/fusion-recipes/attack-types/tiers + cross-file integrity (3×6/5/6/3; 9 recipes; 7 types + 5 armors; 4 tiers; god ID set match between civilizations and recipes). All [PROPOSAL] until balance pass. §2.4a + §5.4 [LOCKED] untouched. ages.json + towers.json + index.html boot loader untouched per alongside pattern; running prototype boots cleanly. cascade-lint clean. Single consolidated commit `a433703` + dual-push. Next step queued: **C2 — Silhouette 5→3 SVG migration** (gated by cultural-sensitivity pass on pose-lock; abstract-placeholder geometric shapes acceptable for harness). PM go required before C2.

*Prior pointer block archived to [`CASCADE-history.md`](CASCADE-history.md).*

**Next planned work:**
1. **C2 — Silhouette 5→3 SVG migration** [next on PM "go"; cultural-sensitivity pose-lock gate via Follow-up #5; abstract geometric placeholder acceptable for harness].
2. C3 → C5 + C7.a → C7.b → C6 per amended order, one PM gate per step.
...
*Document version: 0.30 — 2026-04-29 (continuation): C1 Data layer reshape LANDED via alongside-and-non-breaking pattern. 4 new prototype data files (civilizations / fusion-recipes / attack-types / tiers); commanders.json gained 3 civ commanders alongside legacy 5 (8 playable). Legacy ages.json + towers.json + index.html boot loader untouched; running prototype boots cleanly. cascade-lint extended with 4 new schemas + cross-file integrity; clean. Pointer rewritten; prior 2026-04-29 ratification pointer block archived to [`CASCADE-history.md`](CASCADE-history.md). Next step C2 — Silhouette 5→3 SVG migration (PM go required; Follow-up #5 cultural-sensitivity pose-lock gate). §2.4a + §5.4 [LOCKED] untouched; locked content-skeleton names untouched. Prior: 0.29 — 2026-04-29: Prototype reshape plan RATIFIED (Proposed → Accepted). [Older version history archived to [`CASCADE-history.md`](CASCADE-history.md).]*

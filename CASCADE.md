...
*Document version: 0.33 — 2026-05-02: C6 slice 1 LANDED (commander-legacy sweep). PM gates: option 4 on C6 sequencing (full-now / accept-playability-gap), then option 1 on scope reality (land safely-deletable subset, queue rest as slice 2). Single commit, dual-pushed. Deleted: 9 C7.a comment blocks (12 surfaces) + silhouetteSVG() + LINEAGE_COLORS + getLineageCss() + commander-a..e + cmdr-stat-tilt/generates HTML/setters. Reshaped: pick-card stat row to civ-shape; commanderSilhouetteSVG/Color simplified to civ-only path. ~350 deletions / ~15 additions. Untouched (slice 2): ages.json/towers.json files + boot loader + BALANCE.TOWERS 5-lineage seeding + age-up/Knowledge UI + 5-lineage tower wiring + status-proc switch + lineageOf/lineageRole/HYBRIDS + reference panel + projectile shapes + tutorial Step 3 + cmdr-portrait/chip-dot/glyph CSS + --sinew/--forge/--crown/--veil tokens + cascade-lint schemas. cascade-lint clean. §2.4a + §5.4 [LOCKED] untouched. Prior: 0.32 — 2026-05-01: C5 + C7.a + C7.b LANDED via alongside-and-non-breaking pattern. [Older version history archived to [`CASCADE-history.md`](CASCADE-history.md).]*

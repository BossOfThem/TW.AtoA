...
**2026-04-27 (continuation — Commander-as-summoned-ability-avatar plan-mode pass, ALL 8 steps LANDED).** Plan-mode design pass executing per HANDOFF directive. **Step 1:** filed [`decisions/2026-04-27-commander-as-summoned-ability-avatar.md`](decisions/2026-04-27-commander-as-summoned-ability-avatar.md) (Accepted, Reversibility **Medium**, 3x debug loop inline, supersedes 2026-04-20 on-field-hero). Decision: Commander is **not on the field by default**; emerges only on cast (short-CD ~1.2s / long-CD ~2.8s / signature ~4.5s), animates to target, retreats. Three-surface model (identity plinth out-of-match / passive pip in-HUD / active cast avatar). Builder unit class separated as degenerate mobile-hero (single anim, no AI, no combat). 30-wave historic match-arc beats (solo-only; round-30 = commander's historic antagonist). Same-day amendment banner captures 8-Q PM refinement: Aztec **pre-contact** (Cortés OUT), Greek antagonist **Xerxes I**, Aztec two-phase **Tlaxcalan→Tezcatlipoca** boss, Norse **Jörmungandr-or-Fenrir** (sub-Q OPEN as Follow-up #11), historicity mixed, myth-mode counterfactual confirmed, Builder labels working, §5.4 Lineages-row deletion confirmed-in-principle (Follow-up #10). **Step 2:** [`decisions/2026-04-20-commander-on-field-hero.md`](decisions/2026-04-20-commander-on-field-hero.md) marked Superseded (frontmatter + inline 🔁 banner; body preserved as historical reasoning + reshape-plan C7 dependency tracker). **Step 3:** [`decisions/2026-04-26-prototype-reshape-plan.md`](decisions/2026-04-26-prototype-reshape-plan.md) extended via "Amendment 2026-04-27" — C4 gains Commander Cast bar (3-button: passive display / short-CD / long-CD-or-signature); new C7.a (gut superseded primitives) + C7.b (cast-emerge pipeline + BuilderUnit class + reduce-motion toggle + walkable-build-grid distinct from enemy-path-grid + additive snapshot fields `commander.castState/castTarget/castTtl` + `builders[]`). Effective execution order: C1 → C2 → C3 → C4 → C5 → C7.a → C7.b → C6. **Step 4:** [`concept/phase-4.md`](concept/phase-4.md) §4.1 amended — new "In-match presence model — summoned-on-cast" subsection (three-surface table + three-tier cast budget + non-goals + fatigue mitigation). Phase-3 §3.2/§3.4 transitively superseded — no separate phase-3 turn owed. **Steps 5-8 LANDED in same-day continuation session:** **Step 5** — `concept/phase-4.md §4.4a` NEW Builder unit class subsection (degenerate mobile-hero spec: single walk anim, no AI, no combat, despawn-on-arrive, concurrency cap 3, 90% refund cancel, walkable-build-grid distinct from enemy-path-grid); `§4.4` line-150 stale on-field-hero sentence redacted (strikethrough + supersede note pointing at 2026-04-27 decision). **Step 6** — `concept/phase-4.md §4.7` "Historic match-arc beats (solo-only)" subsection appended with cross-civ antagonist table: Greek **Xerxes I** (named historic) / Aztec **two-phase Tlaxcalan→Tezcatlipoca** (pre-contact; Cortés OUT) / Norse **Jörmungandr** (locked 2026-04-27 — Follow-up #11 CLOSED; Fenrir reserved for post-launch). Cross-civ tonal arc mortal→bridge→cosmic explicitly labelled intentional, mini-boss cadence noted, cultural-sensitivity gate reaffirmed. **Step 7** — CASCADE pointer (this rewrite) + version v0.26 → v0.27. **Step 8** — PROGRESS continuation note appended; Follow-up #11 closure recorded inline at `decisions/2026-04-27-commander-as-summoned-ability-avatar.md` (both anchor points). §2.4a + §5.4 [LOCKED] untouched. Locked content-skeleton names from 2026-04-25 untouched. Prototype frozen (reshape plan still Proposed). Commits `7fd3be8` (steps 1-4) + handoff bundle + this continuation commit.
...
*Document version: 0.27 — 2026-04-27 (continuation, same-day close): Commander-as-summoned-ability-avatar plan-mode pass ALL 8 steps LANDED — §4.4a NEW Builder unit class + §4.4 line-150 redaction + §4.7 Historic match-arc beats subsection (Xerxes / Tlaxcalan→Tezcatlipoca / Jörmungandr round-30 roster; Follow-up #11 CLOSED inline at the 2026-04-27 decision; Fenrir reserved post-launch); pointer + footer rewrite (this); PROGRESS continuation note. §2.4a + §5.4 [LOCKED] untouched. Prior: 0.26 — 2026-04-27 (continuation): plan-mode pass steps 1-4 of 8 LANDED (decision filed + 2026-04-20 superseded + reshape-plan C4/C7 amendment + phase-4 §4.1 subsection); steps 5-8 queued (now closed in v0.27). [Older version history archived to [`CASCADE-history.md`](CASCADE-history.md).]*

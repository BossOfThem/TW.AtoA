...
**2026-05-06 — Per-civ R2 (Greek profile) LANDED** ([`decisions/2026-05-06-per-civ-r2-greek-profile.md`](decisions/2026-05-06-per-civ-r2-greek-profile.md)) **+ dual-pushed (commit `49c4c54` + cleanup `78cba67`).** 5-field Greek profile bound: `matchup_affinity` STRONG vs Colossal (signature; Divine source-density 4-of-15 + 2 partials + zero Poison liability, strongest civ × armor cell at launch) + STRONG vs Shielded (Piercing + Crushing 7-of-15 paired-counter density, densest of any civ × armor cell) / NEUTRAL-mixed vs Spirit (Arcane + Divine smite favorable, Piercing self-penalizes) / NEUTRAL vs Unarmored (tier-asymmetric — T1-T3 Crushing liability, T4+ Slashing answer) / WEAK vs Beast (signature gap — Fire 1-of-15 + Poison-zero); `identity_hook` one-paragraph synthesis of Leonidas Control + per-tower lockdown signature (cd-1.7s mid / range-4.0 mid / dps-centered ±10% / Divine source-density 4-of-15 + 2 partials heaviest) + status-proc archetype kinetic-suppression with **deceleration-weighted lifespan-compounding** shape — falsifiable proc-shape distinction vs predicted Aztec kill-multiplication-weighted + Norse summon-cleave-propagation-weighted; 3x debug loop on identity_hook construction validated; `signature_creep_types` Colossal + Shielded armor-class grain pre-§4.7-lock; `multi_civ_play_hook` Poison-zero structural type-hole → Beast matchup gap filled by Aztec Poison-stacking cross-civ + Slashing-pre-T4 hole filled by Norse Slashing-density (5-of-15 densest); `commander_synergy` Leonidas Control three-stage suppression cycle (Spartan-Training civ-aura passive baseline / This Is Sparta! 4s active spike / The Last Stand 12s zone-denial signature window) binding `controlled_by: leonidas` all 15 Greek rows scaling cleanly per-tower → per-civ aggregate. Cross-civ parity hook → R3 Aztec predicts Beast/Spirit specialist + Crushing-coverage hole + kill-multiplication identity_hook + Montezuma II yield-cycle three-stage commander_synergy. Hard guards verified: §5.4 + §2.4a [LOCKED] untouched / 2026-04-25 content-skeleton verbatim / 2026-04-26 attack-type lock zero deviation / 2026-05-05 R5 DPS ladder cited only / per-commander R5 lane locks no cross-civ pollution / per-tower R1-RN magnitude matrix verbatim / §4.7 OPEN signature_creep_types stays at archetype-grain / Cultural-sensitivity Follow-up #5 hard-gate civ-neutral effect-type language throughout. Mistake flagged: cleanup commit `78cba67` removes accidentally-committed `.accord/` session telemetry (69MB) from tracking + adds to `.gitignore`; blob remains in history at `49c4c54` (purging requires destructive force-push, deferred to PM call). Cascade-lint clean except pre-existing phase-4 629/600 soft-cap (carried). Remaining queue: R3 Aztec / R4 Norse / RN cross-civ × cross-field audit + `phase-4.md §4.x` per-civ specialization body + possible §4.8 exit-gate item add. PM autonomy mandate carries forward.

*Prior 2026-05-06 Per-civ R1 scope LANDED pointer + per-tower arc CLOSED pointer + earlier pointers archived to [`CASCADE-history.md`](CASCADE-history.md).*
...
|------|----------|---------------|
| 2026-05-06 | [Per-civ R2 — Greek profile: 5 fields (matchup_affinity STRONG vs Colossal+Shielded / NEUTRAL-mixed vs Spirit / NEUTRAL vs Unarmored tier-asymmetric / WEAK vs Beast — boss-and-elite-wall specialist with Beast-coverage hole; identity_hook one-paragraph synthesis of Leonidas Control + per-tower lockdown signature dps-centered/cd-1.7s/range-4.0/Divine-source-density-4-of-15 + status-proc archetype kinetic-suppression / deceleration-weighted lifespan-compounding shape falsifiable vs Aztec kill-multiplication + Norse summon-cleave-propagation; signature_creep_types Colossal+Shielded armor-class grain pre-§4.7-lock; multi_civ_play_hook Poison-zero-source structural type-hole resolves to Beast matchup gap filled by Aztec Poison-stacking cross-civ + Slashing-pre-T4 hole filled by Norse Slashing-density; commander_synergy Spartan-Training civ-aura + This-Is-Sparta active-spike + Last-Stand 12s zone-denial three-stage suppression cycle binding controlled_by:leonidas all 15 Greek rows) + 3x debug loop on identity_hook construction (deceleration-weighted lifespan-compounding shape held as falsifiable proc-shape distinction vs Aztec/Norse) + cross-civ parity hook → R3 Aztec (Beast/Spirit specialist + Crushing-coverage hole + kill-multiplication identity)](decisions/2026-05-06-per-civ-r2-greek-profile.md) | Medium |
| 2026-05-06 | [Per-civ specialization sub-pass scope: 4-round queue + per-civ deep-dive axis (R1 of 4) — R2 Greek profile / R3 Aztec / R4 Norse / RN cross-civ × cross-field audit + `phase-4.md §4.x` per-civ specialization body + possible §4.8 exit-gate item add; 5-field per-civ schema locked (matchup_affinity vs §4.10 5-armor RPS / identity_hook one-paragraph mechanical thesis / signature_creep_types armor-class grain pre-§4.7-lock / multi_civ_play_hook from per-tower RN type-coverage gaps / commander_synergy lane-lock binding); 3x debug loop on axis (Axis A per-civ deep-dive chosen; Axis B per-axis cross-civ and Axis C single condensed round rejected) with embedded cross-civ parity hook paragraphs at close of R2/R3/R4](decisions/2026-05-06-per-civ-specialization-scope.md) | Medium |
| 2026-05-06 | [Per-tower RN — cross-civ × cross-tier audit + arc close (CLOSES ARC 4/4): 45-tower lock-conformance audit (zero cascade-violations across 4 locks × 45 towers); cross-civ band statistics confirming 3 orthogonal civ-magnitude axes (Greek dps-centered/cd-1.7s/range-4.0/lockdown / Aztec dps-band-high-T2-T3-and-God/cd-1.4s/range-4.0/Poison-Fire-Slashing-bleed-compounding / Norse dps-centered/cd-1.15s-fastest/range-3.5-shortest/Bleed-density-6-of-15-densest); per-civ falsifiable signature confirmation (3/3 PASS); 3x debug loop on arc-as-whole (schema legibly expressed 3 orthogonal civ-archetypes from one shared ladder under all 4 locks); spine-doc edits applied (§4.8 exit-gate item #2 ticked DONE 2026-05-06 + §4.11.5 closing cross-reference); Phase 4 exit-gate items #1+#2 closed](decisions/2026-05-06-per-tower-rn-cross-civ-audit-and-arc-close.md) | Medium |
| 2026-05-06 | [Per-tower R4 — Norse roster: 15 towers (6 T1-T3 + 6 T4 Demigod + 3 God) × 7 fields with civ-identity-hook prose (Norse summon / called-warriors archetype — force-multiplication via cleave-Bleed-propagation through Heathen Warrior summons) + per-tier reading at close + cross-arc parity hook → RN; all rows tag `summon_anchor: ragnar` per Hard-reversibility lane lock; attack_type verbatim-confirms 2026-04-26 mapping; dps within ±20% of 2026-05-05 R5 baseline ladder with dps-band-centered ±10% pattern (matches Greek, distinct from Aztec's mid-tier-elevation); cd-band 0.8–1.8s mainline (median 1.15s — fastest in any civ); range-band 2.5–4.5 cells mainline (median 3.5 — shortest in any civ; Berserker Lodge 2.5 melee-shortest); Bleed-density 6 of 15 towers (densest single-proc concentration: Longhouse + Berserker Lodge + Björn Ironside + Lagertha + Thor + Tyr); status_proc kind locked from 2026-04-26 with T4 intensity bumps + God-tier alternating-per-shot 2-type Fusion model; references Ragnar Great-Heathen-Army signature (8 warriors @ 180 DPS × 12s) + summon-cap signature-window exception (Berserk + GHA stack → temp 16-cap for 12s) flags](decisions/2026-05-06-per-tower-r4-norse-roster.md) | Medium |
| 2026-05-06 | [Per-tower R3 — Aztec roster: 15 towers (6 T1-T3 + 6 T4 Demigod + 3 God) × 7 fields with civ-identity-hook prose (Aztec yield-stack / economy archetype — kill-multiplication via Poison-stacking + Fire-splash + Slashing-bleed) + per-tier reading at close + cross-arc parity hook → R4/RN; all rows tag `economy_target: montezuma_ii` per Hard-reversibility lane lock; attack_type verbatim-confirms 2026-04-26 mapping; dps within ±20% of 2026-05-05 R5 baseline ladder with band-high-at-T2-T3 falsifiable yield-stack signature (4 of 6 mainline at +10%); cd-band 0.8–2.0s (faster-center than Greek's 0.8–2.6s) expressing kill-multiplication-per-second; range-band 3–5.5 cells; status_proc kind locked from 2026-04-26 with T4 intensity bumps + God-tier alternating-per-shot 2-type Fusion model; references Lever-2 Great Sacrifice (300 T lump + permanent +5%) + `bonus_tribute_yield_in_active_zone` + `permanent_tribute_yield_multiplier` flags; explicit acknowledgment Aztec is heaviest cultural-load center (Follow-up #5)](decisions/2026-05-06-per-tower-r3-aztec-roster.md) | Medium |
...
*Document version: 0.68 — 2026-05-06: Per-civ R2 (Greek profile) LANDED ([`decisions/2026-05-06-per-civ-r2-greek-profile.md`](decisions/2026-05-06-per-civ-r2-greek-profile.md), Accepted, Medium; commit `49c4c54` + cleanup `78cba67`, dual-pushed). 5-field Greek profile: matchup_affinity STRONG vs Colossal+Shielded / NEUTRAL-mixed vs Spirit / NEUTRAL vs Unarmored tier-asymmetric / WEAK vs Beast (boss-and-elite-wall specialist with Beast-coverage hole) — bins from per-tower RN type-coverage audit (Divine source-density 4-of-15 + 2 partials strongest civ × armor cell at launch; Piercing+Crushing 7-of-15 paired-counter density densest of any civ × armor cell; Poison-zero structural type-hole + Fire 1-of-15 Beast gap); identity_hook one-paragraph synthesis Leonidas Control + per-tower lockdown signature (cd-1.7s/range-4.0/dps-centered/Divine-source-density-4-of-15 heaviest) + status-proc archetype kinetic-suppression with **deceleration-weighted lifespan-compounding** shape — falsifiable proc-shape distinction vs predicted Aztec kill-multiplication-weighted + Norse summon-cleave-propagation-weighted; signature_creep_types Colossal+Shielded armor-class grain pre-§4.7-lock; multi_civ_play_hook Poison-zero + Slashing-pre-T4 holes resolved by Aztec Poison-stacking + Norse Slashing-density (5-of-15 densest) cross-civ + deferred Foresight-coin / Follow-up #6 patch-1 path; commander_synergy Spartan-Training civ-aura + This-Is-Sparta active-spike + Last-Stand 12s zone-denial three-stage suppression cycle binding `controlled_by: leonidas` all 15 Greek rows. 3x debug loop on identity_hook (deceleration-weighted lifespan-compounding held as falsifiable). Cross-civ parity hook → R3 Aztec predicts Beast/Spirit specialist + Crushing-coverage hole + kill-multiplication identity + Montezuma II yield-cycle three-stage commander_synergy. Mistake flagged: cleanup `78cba67` removes accidentally-committed `.accord/` session telemetry (69MB) from tracking + adds to `.gitignore`; blob remains in history at `49c4c54` (purging requires destructive force-push, deferred to PM call). Hard guards verified: §5.4 + §2.4a [LOCKED] untouched / 2026-04-25 content-skeleton verbatim / 2026-04-26 attack-type lock / 2026-05-05 R5 DPS ladder cited only / per-commander R5 lane locks zero pollution / per-tower R1-RN matrix verbatim / §4.7 OPEN archetype-grain held / Cultural-sensitivity Follow-up #5 hard-gate civ-neutral effect-type language. Cascade-lint clean except pre-existing phase-4 soft-cap (629/600, carried). Decisions table row added (76 rows). PM autonomy mandate carries forward. HANDOFF flipped — primary directive R3 Aztec profile autonomously. PROGRESS session log appended (3 entries preserved; Phase C archived). CASCADE pointer single block (Per-civ R1 scope LANDED block archived to history); footer trimmed (0.66 archived to history). Prior: 0.67 — 2026-05-06: Per-civ specialization sub-pass OPENED. R1 scope decision LANDED ([`decisions/2026-05-06-per-civ-specialization-scope.md`](decisions/2026-05-06-per-civ-specialization-scope.md)). PM picked Recommended both via AskUserQuestion: Axis A per-civ deep-dive + 5-field schema lock. 4-round queue R2 Greek / R3 Aztec / R4 Norse / RN cross-civ × cross-field audit. 5-field schema (matchup_affinity / identity_hook / signature_creep_types / multi_civ_play_hook / commander_synergy) bound mechanical-content only. 3x debug loop on axis (Axis B per-axis cross-civ and Axis C single condensed round rejected; Axis A chosen with embedded cross-civ parity hook paragraphs). Hard guards: 2026-04-25 content-skeleton verbatim / 2026-04-26 attack-type lock / 2026-05-05 R5 DPS ladder read-only / per-commander R1-R5 lane locks (Hard) / per-tower R1-RN matrix read-only / §4.7 OPEN — signature_creep_types stays at armor-class archetype-grain. Cultural-sensitivity Follow-up #5 hard-gate continues; civ-neutral effect-type language throughout. Inventory: 3 civs × 5 fields = 15 cells. Dual-push points: after R2 / after R3 / after RN. PM autonomy mandate carries. CASCADE pointer flipped (per-tower CLOSED block archived to history). HANDOFF flipped — primary directive R2 Greek profile autonomously. Decisions table row added (75 rows). Footer trimmed (0.65 archived to history). Older version history (0.66 / 0.65 / 0.64 / 0.63 / 0.62 / 0.61 / 0.60 / 0.59 / 0.58 / 0.57 / 0.56 / 0.55 / 0.54 / 0.53 / 0.52 / 0.51 / 0.50 / 0.47 / etc.) archived to [`CASCADE-history.md`](CASCADE-history.md).*

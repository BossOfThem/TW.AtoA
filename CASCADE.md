...
**2026-05-06 — §4.4 mobile unit control sub-pass R2 LANDED (control-model direction lock).** Option (i) waypoint + mode-flags ratified; (ii)/(iii) rejected with R1-steelman reasons echoed (collapsed-engagement-grammar / on-field-hero adjacency). Three bodies authored at spec-level grain: (1) **Mode-flags catalog** = 4 flags (Patrol / Follow-Lane / Hold / Attack-Move; default Follow-Lane; Attack-Move is one of the four, not a separate command); (2) **Engagement-defaults shape** = `R_engage` auto-engage radius + `HP_fall_back` fall-back-at-HP% threshold + `Hold_fire` binary toggle + aggro-priority shape (RPS-strong → low-HP → nearest, with R3-falsification gate for tie-breaker reform); (3) **Control-surface 4-tier scaling principle** = the surface DOES NOT scale — same flags + same defaults + same toggle at T1/T2/T3/T4; tier expresses through stats + signature passive framing, never through new buttons. Negative space verbatim: no per-target manual aggro / no formation commands / no active-pause RTS surface / no per-unit retreat micro / no ability buttons at any tier. 3x debug loop validated catalog count (4 flags exhaustive at §4.4-prose grain) + Hold_fire-as-separate-toggle (cycle decoupling reduces total surface) + aggro-brittleness gate handed to R3. Cross-property coherence hook to R3 explicit (R3 enumerates Body-1 catalog verbatim + Body-2 rules verbatim + authors per-tier multiplier ladder against §4.11.2 (f); Body-3 no-tier-scaling principle applies to control surface only, not stats). Hard guards verified untouched: §5.4 + §2.4a [LOCKED] / 2026-04-25 content-skeleton (no civ/tower/commander/boss names appear) / 2026-04-26 attack-type lock (RPS read-only) / §4.1 commander-as-summoned-ability-avatar (Commander explicitly excluded) / §4.4a Builder (explicitly excluded) / §4.11.2 (f) magnitudes (R3 territory) / §3.4 floor (Body-3 makes no-tier-scaling load-bearing) / per-commander R1-R5 lane locks / per-tower R1-RN / per-civ R1-RN / three-civ-three-equation-form fingerprint (civ-neutral by construction) / §4.7 Option C + R11 grammar / Cultural-sensitivity Follow-up #5. R3 next: spec-grammar bodies (movement-speed-per-tier shape + pathing-mode catalog + engagement-rule catalog + per-rule falsification at all 4 tiers). PM autonomy mandate carries. ([`decisions/2026-05-06-mobile-unit-control-r2-direction-lock.md`](decisions/2026-05-06-mobile-unit-control-r2-direction-lock.md))
...
*Document version: 0.78 — 2026-05-06: §4.4 mobile unit control sub-pass R2 LANDED (control-model direction lock). Option (i) waypoint + mode-flags ratified absent surprise per R1 steelman; (ii)/(iii) rejected. Three bodies authored at spec-level grain: 4-flag mode catalog (Patrol/Follow-Lane/Hold/Attack-Move) + 3-default engagement shape (`R_engage` + `HP_fall_back` + `Hold_fire` + RPS-strong/low-HP/nearest aggro priority) + no-tier-scaling principle (T1-T4 share identical control surface; tier expresses through stats + passive framing only, never new buttons). Cross-property coherence hook to R3 explicit. Hard guards verified untouched verbatim. R3 next: spec-grammar bodies (per-tier speed multiplier ladder against §4.11.2 (f) + pathing-mode catalog + engagement-rule catalog + per-rule §3.4 floor falsification at all 4 tiers). Prior 0.77 — §4.4 R1 scope landed. Older versions archived to [`CASCADE-history.md`](CASCADE-history.md).*

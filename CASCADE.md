...
**2026-05-06 — Monetization specifics ratification LANDED (§4.8 exit-gate item #8 ✅ DONE).** Single-doc audit-and-ratify over the most-locked surface in the codebase (non-negotiable in README.md + identity-constraint in Phase 2.4 + Phase 7.4 + structural commitment in Phase 3.7 + Phase 5.1 step 7). 4-check cross-coherence audit at zero cascade-violations: (Check 1) DRG-reference structural alignment — 6/6 elements match (premium base $5-15 / free BP earnable / premium BP cosmetic-only / cosmetic DLC / no lootboxes / no gameplay-advantage purchase); zero deviations from locked reference. (Check 2) Aux economy ≠ monetization surface — 7/7 aux slots (Damage Bonus / Economy Bonus / Tower-count / Send-Creeps / Send-for-Interest / Call-for-Help / Maze Stone) gated on in-match Tribute (kill-driven per R3) or in-match Divinity (boss-drop / skill-bar per R10), never on premium currency or paid unlock; aux economy is gameplay-balance surface, not commercial surface. (Check 3) Commander cosmetic-slot floor preserves no-p2w — every paid surface (Portrait / Silhouette / Voice lines additional VO) is UI/visual/audio additive; every gameplay-affecting surface (3-ability kit / civ affinity / 20-level ladder) locked behind XP-only earned through play with no purchase shortcut; "**No gameplay-power gates**" tag at §4.0 line 31 verbatim. (Check 4) Phase 7.4 identity-constraint resilience — 4 canonical pressure-source attacks defended by structural locks (launch-revenue maximization / live-ops cadence / competitor-feature pressure / whale-spend optimization). **Reversibility split**: model-shape Hard-class (equivalent to reopening Phase 2 non-negotiables; gacha / lootbox / gameplay-advantage / commander-paywall would trigger this); commercial price-point selection within $5-15 envelope + BP season cadence + cosmetic DLC catalog cadence are Easy-class PM-decisional surface that does not reopen the model. **Spine-doc edits**: §4.8 exit-gate item #8 ✅ ticked DONE. **Phase 4 advances to 7 of 11 exit-gate items closed** (#1 commander / #2 per-tower / #3 per-civ / #4 enemy direction / #5 mobile unit control / #7 economy paper-balance / **#8 monetization specifics**). 4 items remaining (#6 Fusion-numerics / #9 Engine lock / #10 Art director / #11 Cultural-sensitivity Phase-4-exit). Hard guards verified untouched verbatim. PM autonomy mandate carries. ([`decisions/2026-05-06-monetization-specifics-ratification.md`](decisions/2026-05-06-monetization-specifics-ratification.md))
...
**Next planned work (Phase-4 exit):** Fusion-numerics balance-pass (#6) · Engine choice lock (Godot 4 leading; #9 PM-decisional) · Art director engagement (#10 PM-decisional) · Cultural-sensitivity Follow-up #5 consultation (#11 locked to last exit-gate item) · Phase 5 readiness gate (telemetry + wave variance).
...
*Document version: 0.82 — 2026-05-06: Monetization specifics ratification LANDED (§4.8 item #8 ✅ DONE). Single-doc 4-check cross-coherence audit at zero cascade-violations over the most-locked surface in the codebase: DRG-reference structural alignment (6/6 elements match); aux economy ≠ monetization surface (7/7 aux slots in-match-currency-gated); commander cosmetic-slot floor preserves no-p2w (every paid surface UI/visual/audio additive); Phase 7.4 identity-constraint resilience (4 canonical pressure-source attacks defended). Reversibility split: model-shape **Hard-class** (gacha/lootbox/p2w would trigger Phase 2 non-negotiables reopen); commercial price-point + BP cadence + DLC catalog cadence **Easy-class** PM-deferred. Phase 4 advances to **7 of 11 exit-gate items closed**. PM autonomy mandate carries. Prior 0.81 — Economy paper-balance ratification. Older versions archived to [`CASCADE-history.md`](CASCADE-history.md).*

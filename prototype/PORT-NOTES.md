...
| Commander = HUD plinth (top-left identity card) + invisible board-wide passive (per-tower icon pip overlay) + summoned-on-cast avatar (short-CD ≤1.2s emergence, long-CD, signature) — **Commander is NOT on the field by default.** Tower construction handled by a separate civ-coded **Builder unit class** (Greek Mason Crew / Aztec Priest-Builder / Norse Thrall Gang) — degenerate mobile units (no AI, no combat). | `CommanderHUD.tscn` plinth `Control` (portrait + name + level + XP); per-tower passive pip = small icon scene parented under each `Tower.tscn`; summoned avatar = `CommanderAvatar.tscn` (`Node2D` + `Sprite2D` + `AnimationPlayer`) instanced at cast target, animates emerge → act → recede, then `queue_free()`. `Builder.tscn` = `CharacterBody2D` + single anim, spawn-walk-despawn lerp on the build-grid (NOT enemy path); concurrency cap 3 [PROPOSAL]; 90% refund pre-50% cancel. Snapshot additive: `builders:[{x,y,target,ttl}]`; commander state stays HUD-side except during summon TTL. | **SUPERSEDES** the 2026-04-20 persistent on-field-hero shape (1.4× silhouette + 2-cell aura + Shift/C+click + non-destructible knock-back) — see [`decisions/2026-04-27-commander-as-summoned-ability-avatar.md`](../decisions/2026-04-27-commander-as-summoned-ability-avatar.md). Do NOT port the on-field-hero variant; it is parked as a future "Hero-Mode" game variant. Signature `rpc("fire_signature")` host-authoritative. Reduce-motion path turns short-CD into a non-avatar VFX burst. Solo-only historic match-arc (three 10-wave acts; round-30 antagonist per commander) hooks into the long-CD/signature surfaces. |
...
## Source-of-truth when prototype state diverges

**Added 2026-04-26 from a swarm-review pass.** When the same fact is encoded in more than one place, this table is authoritative for "which copy does the port follow":

| Concern | Authoritative source | Drift hazard |
|---|---|---|
| Balance constants (HP_PER_WAVE, base values, etc.) | `prototype/data/balance.json` | The inline `BALANCE` region in `index.html` is loaded from this JSON at boot. If they diverge, JSON wins. Port from JSON. |
| Commander roster (which commanders are playable, their epithet/civ/lore) | `prototype/data/commanders.json` | `Profile.login()` in `index.html:838-875` seeds `unlocks` and `commanderProgress` for `commander-a`..`commander-e` (legacy ids). Those entries are vestigial — the live roster is `commanders.json` filtered by `playable: true`. The legacy id remap (e.g. `index.html:1051, 1086`) exists only to render historical `Profile.data` from old saves; do not port the remap. |
| Tower stats / lineage data | `prototype/data/towers.json` (when present) + `index.html` derive tables | Derived tables are recomputed at `initMatch` from JSON; same authority rule — JSON wins, derive recomputes. |
| Wave structure | `index.html` `WAVE_DEFS` (computed) — driven by `balance.json`'s wave/age multipliers + match.waveCount | The compounding model is the spec. Port the multipliers, not the unrolled table. |
| Mode availability + tags | `index.html` `#scene-mode-select` markup is currently authoritative (Phase 10.x in-flight tags live there) | Once modes stabilize, lift to a JSON descriptor; tagged in this row so the port doesn't bake "Phase 10.3 — lobby opens" copy into engine code. |
| First-run flow gating (which menu buttons show when) | `index.html` `refreshMenu()` (l.1034-1062) | Logic depends on `Profile.data.lastMode` being truthy. As of 2026-04-26 fix, `selectCommander()` seeds `lastMode = "skirmish"` so first-run users see the Play button on return-to-menu. Port: equivalent gating in Godot main-menu controller. |
